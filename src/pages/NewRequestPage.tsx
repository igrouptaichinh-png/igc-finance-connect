...
import { ArrowLeft, ArrowRight, CalendarDays, Check, FileCheck2, Info, Send } from 'lucide-react'
...
export function NewRequestPage() {
  ...
  const { createRequest, topics: requestTypes, isLoading } = useRequests()
  const { user } = useAuth()
  ...
  const availableTypes = useMemo(() => requestTypes.filter((item) => item.category === category), [category, requestTypes])
  const selectedType = requestTypes.find((item) => item.name === requestType) || availableTypes[0]
  const selectedRequestType = selectedType?.name || ''
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      setError(cause instanceof Error ? cause.message : 'Không thể lưu ý kiến lúc này.')
    } finally {
      setSubmitting(false)
    }
  }
      ...
      <div className="wizard-layout">
        <form className="request-form content-card" onSubmit={submit}>
          ...
          {step === 1 && <div className="form-step">
            ...
            {isLoading ? <div className="account-loading">Đang tải chủ đề từ Supabase...</div> : <div className="category-grid">{requestTypes.map((item) => <button type="button" key={item.category} onClick={() => chooseCategory(item.category)} className={category === item.category ? 'selected' : ''}><i>{categoryIcons[item.category]}</i><span><strong>{item.category}</strong><small>{item.description}</small></span>{category === item.category && <Check className="category-check" size={16} />}</button>)}</div>}
            {selectedType && <label className="field full-field"><span>Nội dung cụ thể</span><select value={selectedRequestType} onChange={(event) => setRequestType(event.target.value)}>{availableTypes.map((item) => <option key={item.name}>{item.name}</option>)}</select><small><Info size={13} />Phản hồi dự kiến trong {selectedType.slaHours} giờ làm việc{selectedType.requiresApproval && ' · Có thể cần thêm bước đánh giá'}</small></label>}
          </div>}
          ...
          {step === 3 && <div className="form-step review-step">
            ...
            <div className="review-grid"><div><span>Chủ đề</span><strong>{category}</strong></div><div><span>Nội dung</span><strong>{selectedRequestType}</strong></div><div><span>Tiêu đề</span><strong>{title || 'Chưa nhập tiêu đề'}</strong></div><div><span>Phòng ban</span><strong>{user?.department}</strong></div><div><span>Mức độ ảnh hưởng</span><strong>{priority}</strong></div><div><span>Phạm vi</span><strong>{visibility}</strong></div></div>
